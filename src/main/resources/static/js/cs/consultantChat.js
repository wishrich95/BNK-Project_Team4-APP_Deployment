document.addEventListener('DOMContentLoaded', function () {
    ...
    // ✅ contextPath 정규화 (중요)
    // - 항상 "/로 시작" + "마지막 /로 끝" 보장
    // - 예) "/busanbank" -> "/busanbank/"
    // - 예) "/busanbank/" -> "/busanbank/"
    // - 예) "/" -> "/"
    // =========================
    let contextPath = window.CTX_PATH || '/';
    ...
    // =========================
    // ✅ 공통 fetch 헬퍼
    // - 404 같은 HTML 응답을 JSON으로 파싱하려다 터지는 문제 방지
    // =========================
    async function fetchJson(url, options = {}) {
        ...
        if (!res.ok) {
            // 에러일 때는 HTML이든 뭐든 텍스트로 읽어서 원인 노출
            const text = await res.text().catch(() => '');
            throw new Error(`HTTP ${res.status} ${res.statusText} :: ${url}\n${text.slice(0, 200)}`);
        }

        // 정상인데도 JSON이 아니면(예: 서버가 뷰 페이지 반환) 즉시 원인 노출
        if (!contentType.includes('application/json')) {
            const text = await res.text().catch(() => '');
            throw new Error(`JSON 아님(content-type=${contentType}) :: ${url}\n${text.slice(0, 200)}`);
        }
    ...
    function appendMessage(text, type) {
        ...
        if (type === 'me') row.classList.add('me');
        else if (type === 'system') row.classList.add('system');
        else row.classList.add('user');
    ...
    function connectWebSocket() {
        ...
        ws.addEventListener('open', () => {
            console.log('📡 CallAgent WS connected:', wsUrl);
    ...
    // =========================
    if (btnAssignNext) {
        btnAssignNext.addEventListener('click', async function () {
            btnAssignNext.disabled = true;

            try {
                const data = await fetchJson(`${contextPath}cs/chat/consultant/assignNext`, {
                    method: 'POST',
                ...
                // 2) 진행중 목록에서 해당 세션 선택
                if (chattingList) {
                    const li = chattingList.querySelector(`li[data-session-id="${sessionId}"]`);
                    if (li) selectSession(sessionId, li);
                    else console.warn('chattingList에서 sessionId에 해당하는 li를 찾지 못했습니다.', sessionId);
                }
    ...
    function updateCurrentSessionLabel() {
        ...
        currentSessionLabel.textContent = currentSessionId ? ('세션 #' + currentSessionId) : '없음';
    }

    function highlightSessionLi(li) {
        if (!li) return;
        if (activeSessionLi) activeSessionLi.classList.remove('is-active');
    ...
    function renderSessionLists(data) {
        ...
        `;
            ...
            if (waitingCountEl) waitingCountEl.textContent = data.waitingList.length + '건';
        }
        ...
        `;
            ...
            if (chattingCountEl) chattingCountEl.textContent = data.chattingList.length + '건';
        }
    }
